...
_DB_KEYS = (
    ...
    "APP_DATABASE_URL", "ADMIN_DATABASE_URL",
)
...
def test_builds_admin_database_url_from_db_env(monkeypatch):
    # Alembic migrations use admin_database_url; it must also compose from DB_* so the
    # migrate task needs only DB_* + DB_PASSWORD (no manual ADMIN_DATABASE_URL).
    _clear(monkeypatch)
    monkeypatch.setenv("DB_HOST", "rds.example.com")
    monkeypatch.setenv("DB_PORT", "5432")
    monkeypatch.setenv("DB_NAME", "saalr")
    ...
    assert s.admin_database_url == (
        "postgresql+asyncpg://saalr_admin:p%40ss%2Fword@rds.example.com:5432/saalr"
    )


def test_admin_database_url_unchanged_without_db_host(monkeypatch):
    _clear(monkeypatch)

    s = Settings(_env_file=None)

    assert s.admin_database_url == "postgresql+asyncpg://postgres:postgres@localhost:5432/saalr"
