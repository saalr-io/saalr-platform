...
// IvCurves renders InfoHint, which uses a react-router <Link>, so it needs a router.
...
describe('IvCurves', () => {
  it('renders the smile and term-structure charts', () => {
    wrap(<IvCurves surface={SURFACE} expiry="2026-07-17" />)
  ...
  it('skips strikes missing a side without crashing', () => {
    ...
    wrap(<IvCurves surface={s} expiry="2026-07-17" />)
  ...
  it('shows an empty state (no crash) for a surface with no expiries', () => {
    wrap(<IvCurves surface={{ ...SURFACE, expiries: [] }} expiry="" />)
