...
CREATE TABLE model_validation_runs (
  ...
  model_name          TEXT NOT NULL,            -- 'garch', 'finbert', 'lstm'
  market              CHAR(2) NOT NULL,         -- 'US' or 'IN'
  cohort_label        TEXT NOT NULL,            -- e.g., 'SPY_QQQ_Q3_2026'
  baseline_name       TEXT NOT NULL,            -- 'hv21', 'arima', etc.
  status              TEXT NOT NULL CHECK (status IN ('running', 'passed', 'failed')),
  metric_summary_json JSONB NOT NULL,           -- mae/mape/lift/t-stat/confidence
  report_uri          TEXT,                     -- immutable published report location (S3/URL)
  started_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  completed_at        TIMESTAMPTZ
);

CREATE INDEX idx_validation_model_market ON model_validation_runs(model_name, market, started_at DESC);
```
...
### 5.4 Get realized performance metrics from transaction history

```http
GET /v1/portfolio/performance?period=30d&strategy_id=str_01HXY...
Authorization: Bearer <jwt>
```
...
```json
{
  ...
  "data_quality": {
    "missing_fills": 0,
    "stale_price_points": 2
  }
}
```

This endpoint is read-only and computed from reconciled executions/positions history.

### 5.5 Get model validation report status

```http
GET /v1/ml/models/{name}/validation?market=US
Authorization: Bearer <jwt>
```
...
```json
{
  ...
  "baseline": "hv21",
  "cohort_label": "SPY_QQQ_Q3_2026",
  "metrics": {
    "delta_mae_vs_baseline": -0.018,
    "t_stat": 2.31
  },
  "report_url": "https://saalr.io/validation/garch-us-q3-2026",
...
- India retail compliance (all India brokers/exchanges): OMS enforces SEBI hard cap of 10 orders/second per client per exchange before broker submission.
- Zerodha: adapter keeps a safety token-bucket at 8/sec to reduce edge-window violations while remaining below SEBI cap.
...
| `RATE_LIMIT_SEBI_ORDER_OPS_EXCEEDED` | 429 | India retail order rate would exceed 10 OPS per client per exchange | none |
| `AUTH_PLUGIN_DB_ACCESS_FORBIDDEN` | 403 | External plugin attempted direct DB access path | investigate |
| `ML_VALIDATION_REPORT_REQUIRED` | 409 | Requested validated-model claim/surface without published OOS report | investigate |
...
INDIA_ORDER_RATE_LIMITS = {
  "sebi_hard_cap_ops": 10,     # per client per exchange
  "internal_target_ops": 8,    # safety margin for burst control
}

PLUGIN_ACCESS_POLICY = {
  "allow_direct_db_queries": False,
...
VALIDATION_POLICY = {
  "allow_validated_claims_without_report": False,
  "required_status_for_validated_claim": "passed"
}
```
...
### 11.4 Architecture constraints (must match Architecture/HLD)
- **Auth:** Clerk is the selected provider for Phase 1. Keep all provider-specific logic behind an internal auth adapter interface.
- **Research service isolation:** `apps/research-agent/` is a separate internal service deployment with independent autoscaling, timeout ceilings, and per-tenant budget controls.
- **Time-series scaling path:** single TimescaleDB instance through Year 1; consider sharding only if at least 2 trigger metrics breach for 4 consecutive weeks:
  - p95 peak CPU > 70%
  - projected storage exhaustion < 90 days
  - p99 market-query latency > 800ms
  - ingestion lag during market hours > 30s
- **Backtesting in Phase 1:** vectorbt via thin wrapper only. Every run must persist seed/version metadata and realism knobs (slippage, commissions, liquidity assumptions) for deterministic replay.
- **India residency (Phase 4+):** India-tenant PII and billing identity data are region-pinned to ap-south-1. Cross-region analytics datasets must use pseudonymized identifiers.
- **Mobile escalation:** keep responsive web as default until one product trigger and one business trigger hold for 8 consecutive weeks.
- **India order-rate compliance:** apply SEBI 10 OPS cap only to India exchanges (`market=IN`); US exchanges are not subject to this SEBI limiter.
- **External plugin data access:** broker/LLM MCP plugins use Saalr-owned read-only APIs/functions only; direct SQL access to production trading DB is disallowed.
- **Phase 1 user scope:** implement retail-trader workflows only (single-user tenant UX); enterprise/B2B/white-label flows are deferred.
- **Claims governance:** any externally-visible "validated" claim must map to a published OOS validation report with `status=passed`.
...
2. **Postgres + TimescaleDB schemas with RLS policies.** Migrations runnable. Deploy as single primary instance for Year 1; add dashboards for scaling-trigger metrics from day one.
...
8. **Backtest engine (vectorbt wrapper) + metrics.** Backtest runs end-to-end with persisted replay metadata (seed/version) and realism controls.
...
16. **Zerodha adapter + India market data + residency foundations.** Second broker, second geography, and ap-south-1 region-pinning controls for India-tenant PII/billing identity.
17. **Research Agent (forked TradingAgents productionized) as separate internal service.** Premium tier launch with independent rate/budget/timeout controls.
