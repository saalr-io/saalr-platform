...
Validation-first operating mode: model and growth claims are gated by measured results.
...
### 3.2 Tech
- **Provider:** Clerk (or Auth0; see ADR-001 below)
...
sequenceDiagram
    ...
    API->>RISK: validate (size, exposure, entitlements)
...
### 10.7 Claims governance

- Capability statuses are tracked as `built`, `code_complete`, or `validated`.
- External claims may use `validated` only when OOS validation reports are published.
- Failed validation downgrades model status and user-facing messaging.
...
**Status:** Accepted
**Decision:** Use Clerk as primary; Auth0 as fallback if Clerk pricing escalates at scale.
**Why:** Auth is a solved problem; self-hosting costs more in security risk than the provider fee. Clerk has B2B support for Year 2 use cases.
...
**Decision:** Run all production infrastructure on AWS. Migrate OptionsWorld and OptionsAcademy from Azure+GCP during Phase 1. Single region us-east-1 for Year 1; consider ap-south-1 (Mumbai) post-seed if DPDP Act enforcement or Indian user latency justifies it.
**Why:**
...
Phase 1 scope constraint: retail users only; no enterprise/B2B/white-label feature commitments in this phase.

| Question | Phase | Default if not resolved |
|----------|-------|------------------------|
| **Q1:** Clerk vs Auth0 final selection | Phase 1 | Clerk (cheaper at <10K users) |
| **Q2:** Real-time data provider for India | Phase 4 | Upstox feed (cheapest with good docs) |
| **Q3:** Mobile-native app trigger | Phase 5 | Defer; responsive web until churn data justifies it |
| **Q4:** Data residency in India under DPDP Act | Phase 4 | Get legal opinion; assume yes until told no |
| **Q5:** Research Agent: open-source LLMs for cost optimization | Phase 5 | Stay on OpenAI/Anthropic until LLM cost > 30% of MRR |
| **Q6:** Reconciliation as separate service | Post-seed | Stays in monolith until OMS latency requires split |
